...
public class CrashAndBonus : MonoBehaviour
{

    public GameObject crashBox;
   ...
   public void CreateUtility()
    {
        ...
        for (float i = 17.5f; i < 250; i += 12.5f)
        {
            ...
            if (choose >= 8 && choose <= 10)
            {
                if (count == 0)
                {
                    var obj = Instantiate(crashBox, new Vector3(Random.Range(-2, 3) * 3, 0.5f, i), Quaternion.identity,transform);
                    obj.transform.GetChild(0).GetComponent<CrashBox>().crashCount = Random.Range(4, 20);
                }
                else if (count == 1)
                {
                    var obj = Instantiate(crashBox, new Vector3(Random.Range(-2, 0) * 3, 0.5f, i), Quaternion.identity, transform);
                    obj.transform.GetChild(0).GetComponent<CrashBox>().crashCount = Random.Range(4, 20);
                    var obj2 = Instantiate(crashBox, new Vector3(Random.Range(1, 3) * 3, 0.5f, i), Quaternion.identity, transform);
                    obj2.transform.GetChild(0).GetComponent<CrashBox>().crashCount = Random.Range(4, 20);
                }
